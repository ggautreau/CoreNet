...
typedef struct
{
    int*        KmaxesV ;   /* values of ex-aequo MAP labels [nk] */
     double*  FkiM;              /*  fk(xi) [npt,nk] */     /*nemometa 0.1*/
...
/* ------------------------------------------------------------------- */
{
  ...
  switch( StatModelP->Spec.BetaModel )
    ...
    case BETA_PSGRAD:
      ...
      Rprintf(  "beta estimation mode %d\n", 
	       StatModelP->Spec.BetaModel ) ;
      Rprintf( "  %6.4f\n", StatModelP->Para.Beta ) ;
...
/* ------------------------------------------------------------------- */
{
  ...
  for ( btatest = 0.0, nbta = 0, stop = FALSE ;
      ...
      if ( sts == STS_OK )
	{
	  ...
	  if ( fcri != NULL )
	    Rprintf( fcri, "%5.2f  %10.1f  %6.3f  %d\n", 
		     btatest , 
...
/* ------------------------------------------------------------------- */
{
    ...
    /* Allocate all working variables */
    {
        ...
        working.FkiM          = GenAlloc( npt * nk, sizeof( double ), 0, func, "FkiM" ) ;
	...
	err = MakeParaFromLabeled( DataP, ClassifM, & StatModelP->Spec, 
	  ...
	  return err ;
        ...
        if ( flog != NULL )
            Rprintf( flog, "Initialization by sorting variable %d:\n",
                     NemParaP->SortedVar + 1 ) ;
    ...
    case INIT_FILE:    /* initial classification is given by a file */
        if ( flog != NULL )
            Rprintf( flog, "Initialization with specified partition :\n" ) ;
    ...
    case INIT_LABEL:
        ...
        if ( flog != NULL ) {
	  Rprintf( flog, "Initialization with partially labeled sample :\n" ) ;
	  Rprintf( flog, "%4d ", 0 ) ;
	}
    ...
    case INIT_MIXFIX:
        ...
        if ( flog != NULL ) {
	  Rprintf( flog, "Initializing parameters from given value :\n" ) ;
	  Rprintf( flog, "%4d ", 0 ) ;
	}
   ...
   *missdP = -1 ;
      ...
      for ( d = 0 ; d < nd ; d ++ )
	{
	  ...
	  if ( ParaP->NbObs_KD[ k * nd + d ] < EPSILON )
	    {
	      Rprintf( "Warning: no data in class k=%d, variable=%d\n" ,
		       k + 1 , d + 1 );
	      *misskP = k ;
	      *missdP = d ;
...
/* ------------------------------------------------------------------- */
{
  float mult; 
        ...
        else
        {
            ...
            Rprintf( (*FlogP), "NEM log file  -  %s\n",
                     asctime( localtime( &timer ) ) ) ;

	    mult = exp( -((int) ( log(Npt/1000.)/log(10) )) * log( 10 ) ) ;
	    Rprintf( (*FlogP), "  Criteria are multiplied by %f\n\n", mult ) ;
        }
}  /* end of StartLogFile() */
...
/* ------------------------------------------------------------------- */
{
  ...
  if ( nbsucc > 0 ) {
    ...
    *CriterP = bestCritS ;
    ...
    if ( Flog != NULL )
       Rprintf( Flog, "Best start was %d (U = %g)\n", 
...
/* ------------------------------------------------------------------- */
{
    ...
    */
          ...
          iter ++ )
    {
        ...
        if ( Flog != NULL ) Rprintf( Flog, "%4d ", iter ) ;
	...
	EstimBeta( SpecP->BetaModel, &NemParaP->BtaPsGrad, SpatialP, CM,
        ...
        else if ( err == STS_W_EMPTYCLASS ) /* Else -> class emptyk is empty */
        {
            Rprintf(  "Class %d empty at iteration %d\n", 
                     emptyk, iter ) ;
            if ( Flog != NULL )
               Rprintf( Flog, " Class %d empty at iteration %d\n", 
                        emptyk, iter ) ;
        }
    ...
    if ( iter == 0 )       /* model parameters estimation not yet done */
    {
        ...
        ComputePkFkiM( DataP, SpecP, ParaP, 
		       WorkP->FkiM,WorkP->PkFkiM, WorkP->LogPkFkiM ) ;
    }
...
/* ------------------------------------------------------------------- */
{
    ...
    Rprintf( Flog, "%4s  %5s %5s %5s", "It", "UM", "PM", "Er" ) ;
    for ( it = 0 ; it < NbEIters ; it ++ )
    {
        Rprintf( Flog, " %3s%-2d %3s%-2d %3s%-2d", 
		 "UE", it+1, "PE", it+1, "Er", it+1 ) ;
    }

    Rprintf( Flog, " " ) ;

    Rprintf( Flog, " %5s", "Beta" ) ;

    Rprintf( Flog, " " ) ;

    for ( k = 0 ; k < SpecP->K ; k ++ )
    {
        Rprintf( Flog, " %3s%02d", "P", k + 1 ) ;
    }
    ...
    for ( k = 0 ; k < SpecP->K ; k ++ )
    {
        for ( d = 0 ; d < Nd ; d ++ )
        {
            Rprintf( Flog, " %3s%02d_%1d", "M", k + 1, d + 1 ) ;
        }
    }
    ...
    for ( k = 0 ; k < SpecP->K ; k ++ )
    {
        for ( d = 0 ; d < Nd ; d ++ )
        {
            Rprintf( Flog, " %3s%02d_%1d", "D", k + 1, d + 1 ) ;
        }
    }
    ...
    for ( k = 0 ; k < SpecP->K ; k ++ )
    {
        for ( d = 0 ; d < Nd ; d ++ )
        {
            Rprintf( Flog, " %3s%02d_%1d", "n", k + 1, d + 1 ) ;
        }
    }
...
/* ------------------------------------------------------------------- */
{
    ...
    for ( k = 0 ; k < SpecP->K ; k ++ )
    {
        Rprintf( Flog, " %5.3f", ParaP->Prop_K[ k ] ) ;
    }
    ...
    for ( k = 0 ; k < SpecP->K ; k ++ )
    {
        for ( d = 0 ; d < Nd ; d ++ )
        {
            Rprintf( Flog, " %7.3f", ParaP->Center_KD[ (k*Nd) + d ] ) ;
        }
    }
    ...
    for ( k = 0 ; k < SpecP->K ; k ++ )
    {
        for ( d = 0 ; d < Nd ; d ++ )
        {
            Rprintf( Flog, " %7.3f", ParaP->Disp_KD[ (k*Nd) + d ] ) ;
        }
    }
    ...
    for ( k = 0 ; k < SpecP->K ; k ++ )
    {
        for ( d = 0 ; d < Nd ; d ++ )
        {
            Rprintf( Flog, " %7.1f", ParaP->NbObs_KD[ (k*Nd) + d ] ) ;
        }
    }
 ...
 )
{
  ...
  */
  ...
  for ( k = 0, cumnum = 0.0 ; k < Nk ; k ++ ) {
    ...
    Cinum_K[ k ] = PkfkiM[ ( Ipt * Nk ) + k ] *   exp( (double) ParaP->Beta * context ) ;

    cumnum = cumnum + Cinum_K[ k ] ;
    if (Cinum_K[ k ] >= Cinum_K[ kstar ] ) {kstar=k;}
  ...
  if (cumnum>MAXFLOAT) {
    ...
    Cinum_K[ kstar ] =MAXFLOAT;
  }  /*end du if (cumnum>MAXFLOAT) */
...
/* ------------------------------------------------------------------- */
{
    ...
    CriterP->P = 0.0 ; /* P =sum[i] log(sum[k] fki (exp(bta * sum[j~i] wij cjk))/sum[k] (exp(bta * sum[j~i] wij cjk)) */

    /* For each point i */
    for ( i = 0 ; i < Npt ; i ++ )
    {
        double fi ;  /* fi = sum[k] pkfki */
	double zi ;  /* zi = sum[k] exp(bta * sum[j~i] wij cjk) */
	double pi ;  /* pi = sum[k] fki (exp(bta * sum[j~i] wij cjk))/sum[k] (exp(bta * sum[j~i] wij cjk)) */

        /* Get point i's neighbors */
        int nbn = fGetNeigh( i, & SpatialP->NeighData, &(WorkP->Neighs) ) ;

        /* For each class k */
        for ( k = 0 , fi = 0.0, zi = 0.0 , pi=0.0; 
	      k < Nk ; 
	      k ++ )
        {
            float   cik      = CM[ i * Nk + k ] ;
	    ...
	    zi = zi + exp( Beta * pik ) * WorkP->PkFkiM[ i * Nk + k ] / WorkP->FkiM[ i * Nk + k ] ;
            zi = zi < MAXFLOAT ?  zi : MAXFLOAT; 

            pi = pi+  WorkP->PkFkiM[ i * Nk + k ] *  exp( Beta * pik ) ;
            pi = pi < MAXFLOAT ?  pi : MAXFLOAT; 

	    //Rprintf("pi %f et fk % f et  exp(betapik %f  \n",pi, WorkP->FkiM[ i * Nk + k ] , exp( Beta * pik ) );


        } /* end   For each class k */
        pi = pi / zi;
        CriterP->P= CriterP->P + log(pi);
